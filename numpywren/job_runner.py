        ...
        for expr_idx, var_values in operator_refs:
            ...
            try:
                if (node_status == lp.NS.READY or node_status == lp.NS.RUNNING):
                    ...
                    next_operator, log_bytes = await self.loop.run_in_executor(computer, self.program.post_op, expr_idx,
                                                                               var_values, lp.PS.SUCCESS, inst_block)
...
def lambdapack_run_with_failures(failure_key, program, pipeline_width=5, msg_vis_timeout=60, cache_size=0, timeout=200,
                                 idle_timeout=5, msg_vis_timeout_jitter=15, compute_threads=1):
    program.incr_up(1)
    ...
    for i in range(pipeline_width):
        ...
        coro = lambdapack_run_async(loop, program, computer, cache, shared_state=shared_state, timeout=timeout,
                                    msg_vis_timeout=msg_vis_timeout, msg_vis_timeout_jitter=msg_vis_timeout_jitter)
...
# @profile
def lambdapack_run(program, pipeline_width=5, msg_vis_timeout=60, cache_size=5, timeout=200, idle_timeout=5,
                   msg_vis_timeout_jitter=15, compute_threads=1):
    program.incr_up(1)
    lambda_start = time.time()
    ...
    # client = boto3.client('s3', region_name=config['account']['aws_region'])
...
async def reset_msg_visibility(msg, queue_url, loop, timeout, timeout_jitter, lock):
    ...
    while (lock[0] == 1 and loop.is_running()):
        try:
            ...
            sqs_client = boto3.client('sqs', use_ssl=False,region_name='us-east-1')
            res = sqs_client.change_message_visibility(VisibilityTimeout=60, QueueUrl=queue_url,
                                                                 ReceiptHandle=receipt_handle)
            # async with boto3.client('sqs', use_ssl=False,
            #                                  region_name='us-east-1') as sqs_client:
            #     res = await sqs_client.change_message_visibility(VisibilityTimeout=60, QueueUrl=queue_url,
            #                                                      ReceiptHandle=receipt_handle)
            num_tries += 1
...
# @profile
async def lambdapack_run_async(loop, program, computer, cache, shared_state, read_queue, pipeline_width=1,
                               msg_vis_timeout=60, timeout=200, msg_vis_timeout_jitter=15):
    # global REDIS_CLIENT
    REDIS_CLIENT = boto3.client('s3')
    # session = aiobotocore.get_session(loop=loop)
    ...
    try:
        while (loop.is_running()):
            ...
            lp.incr("{0}_busy".format(program.hash))
            ...
            lp.put(msg["MessageId"], str(time.time()))
            print("creating lock")
            lock = [1]
            ...
            shared_state["busy_workers"] -= 1
            lp.decr("{0}_busy".format(program.hash),1)
