...
class RowPivotedBigMatrix(BigMatrix):
    def __init__(self, matrix, row_permutation_dict):
        ...
        self.matrix = matrix
        super().__init__(self, key=matrix.key, shape=matrix.shape, shard_sizes=matrix.shard_sizes, bucket=matrix.bucket,
                         prefix=matrix.prefix, dtype=matrix.dtype, parent_fn=matrix.parent_fn,
                         write_header=matrix_write_header, autosqueeze=matrix.autosqueeze, lambdav=matrix.lambdav,
    ...
    async def __s3_key_to_row__(self, key, row_idx, loop=None):
        ...
        while bio is None and n_tries <= max_n_tries:
            try:
                ...
                print("get:"+self.bucket+"/"+key)
                header_resp = client.get_object(Bucket=self.bucket, Key=key, Range=header_range_query)["Body"]
                # async with resp["Body"] as stream:
                #     header_len_bytes = await stream.read()
                header_len_bytes = resp['Body'].read()
                ...
                print("get:"+self.bucket+"/"+key)
                resp = client.get_object(Bucket=self.bucket, Key=key, Range=row_range_query)
                matrix_bytes = resp['Body'].read()
                # async with resp['Body'] as stream:
                #     matrix_bytes = await stream.read()
