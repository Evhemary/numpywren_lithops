...
def run_experiment(problem_size, shard_size, pipeline, num_priorities, lru, eager, truncate, max_cores, start_cores,
                   trial, launch_granularity, timeout, log_granularity, autoscale_policy, standalone, warmup, verify,
                   matrix_exists, read_limit, write_limit, n_threads):
    # set up logging
    ...
    arg_bytes = pickle.dumps((problem_size, shard_size, pipeline, num_priorities, lru, eager, truncate, max_cores,
                              start_cores, trial, launch_granularity, timeout, log_granularity, autoscale_policy,
    ...
    all_futures = pwex.map(
        lambda x: job_runner.lambdapack_run(program, pipeline_width=pipeline_width, cache_size=cache_size,
                                            timeout=timeout), range(start_cores), extra_env=extra_env)
    ...
    try:
        while program.program_status() == lp.PS.RUNNING:
            ...
            for i, queue_url in enumerate(program.queue_urls):
                ...
                attrs = client.get_queue_attributes(QueueUrl=queue_url, AttributeNames=['ApproximateNumberOfMessages',
            ...
            obj = get("{0}_busy".format(program.hash),s3=False)
            busy_workers = obj['Item']['nums']
            obj = get("{0}_write_sparse".format(program.hash),s3=False)
            sparse_writes = parse_int(obj['Item']['nums']) / 1e9
            ...
            item = program.get_flops()
            current_gflops = item['Item']['nums']
            ...
            item = program.get_read()
            current_gbytes_read = item['Item']['nums']
            ...
            item = program.get_write()
            current_gbytes_write = item['Item']['nums']
            ...
            if (len(flops) > smooth_len + 5):
                ...
                read_rate_5_min_window = (read_objects[-1] - read_objects[-smooth_len]) / (
                            times[-1] - times[-smooth_len])
                write_rate_5_min_window = (write_objects[-1] - write_objects[-smooth_len]) / (
                            times[-1] - times[-smooth_len])
            ...
            # read_timeouts = int(parse_int(get("s3.timeouts.read")))
            # write_timeouts = int(parse_int(get("s3.timeouts.write")))
            # redis_timeouts = int(parse_int(get("redis.timeouts")))
            ...
            print(
                "{0}: Total GFLOPS {1}, Total GBytes Read {2}, Total GBytes Write {3}, Total Gbytes Write Sparse : {4}".format(
                    curr_time, current_gflops, current_gbytes_read, current_gbytes_write, sparse_writes))
            print(
                "{0}: Average GFLOPS rate {1}, Average GBytes Read rate {2}, Average GBytes Write  rate {3}, Average Worker Count {4}".format(
            ...
            print(
                "{0}: smoothed GFLOPS rate {1}, smoothed GBytes Read rate {2}, smoothed GBytes Write  rate {3}, smoothed Worker Count {4}".format(
                    curr_time, gflops_rate_5_min_window, gread_rate_5_min_window, gwrite_rate_5_min_window,
                    workers_5_min_window))
            print("{0}: smoothed read txns/s {1}, smoothed write txns/s {2}".format(curr_time, read_rate_5_min_window,
                                                                                    write_rate_5_min_window))
            # print("{0}: Read timeouts: {1}, Write timeouts: {2}, Redis timeouts: {3}  ".format(curr_time, read_timeouts,
            #                                                                                    write_timeouts,
            #                                                                                    redis_timeouts))
            # print("{0}: Read timeouts fraction: {1}, Write timeouts fraction: {2}".format(curr_time,
            #                                                                               read_timeouts_fraction,
            #                                                                               write_timeouts_fraction))
            print("=======================================")

            time_since_launch = time.time() - last_run_time
            if (autoscale_policy == "dynamic"):
                if (time_since_launch > launch_granularity and up_workers < np.ceil(
                        waiting * 0.5 / pipeline_width) and up_workers < max_cores):
                    ...
                    new_futures = pwex.map(lambda x: job_runner.lambdapack_run(program, pipeline_width=pipeline_width,
                                                                               cache_size=cache_size, timeout=timeout),
                                           range(cores_to_launch), extra_env=extra_env)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--max_cores', type=int, default=16)
    parser.add_argument('--start_cores', type=int, default=16)
    ...
    run_experiment(args.problem_size, args.shard_size, args.pipeline, args.num_priorities, args.lru, args.eager,
                   args.truncate, args.max_cores, args.start_cores, args.trial, args.launch_granularity, args.timeout,
                   args.log_granularity, args.autoscale_policy, args.standalone, args.warmup, args.verify,
                   args.matrix_exists, args.write_limit, args.read_limit, args.n_threads)
