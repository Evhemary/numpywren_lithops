...
def run_experiment(problem_size, shard_size, pipeline, num_priorities, lru, eager, truncate, max_cores, start_cores,
                   ...
                   matrix_exists, read_limit, write_limit, compute_threads_per_worker):
    # set up logging
    ...
    if standalone:
        extra_env = {"AWS_ACCESS_KEY_ID": os.environ["AWS_ACCESS_KEY_ID"].strip(),
                     "AWS_SECRET_ACCESS_KEY": os.environ["AWS_SECRET_ACCESS_KEY"].strip(), "OMP_NUM_THREADS": "1",
    ...
    try:
        while (program.program_status() == lp.PS.RUNNING):
            ...
            obj = get("{0}_repeated_compute".format(program.hash),s3=False)
            repeated_compute = parse_int(obj['Item']['nums'])

            obj = get("{0}_repeated_post_op".format(program.hash),s3=False)
            repeated_post_op = parse_int(obj['Item']['nums'])

            obj = get("{0}_repeated_finish".format(program.hash),s3=False)
            repeated_finish = parse_int(obj['Item']['nums'])

            obj = get("{0}_not_ready".format(program.hash),s3=False)
            not_ready = parse_int(obj['Item']['nums'])
            ...
            print(
                "{0}: Total GFLOPS {1}, Total GBytes Read {2}, Total GBytes Write {3}".format(curr_time, current_gflops,
                                                                                              current_gbytes_read,
            ...
            if (time_since_launch > (0.85 * timeout)):
                ...
                new_futures = pwex.map(
                    lambda x: job_runner.lambdapack_run(program, pipeline_width=pipeline_width, cache_size=cache_size,
                                                        timeout=timeout), range(cores_to_launch), extra_env=extra_env)
...
if __name__ == "__main__":
    ...
    run_experiment(args.problem_size, args.shard_size, args.pipeline, args.num_priorities, args.lru, args.eager,
                   ...
                   args.matrix_exists, args.write_limit, args.read_limit, args.compute_threads_per_worker)
